...
dict_dir=/kaldi/egs/king3
dict=dic5k.formatted.txt

echo "=== Preparing the dictionary ..."
if [ ! -f $locdict/$dict ]; then
	mkdir -p $locdict 
	echo "copy $dict_dir/$dict to $locdict"
	cp -p $dict_dir/$dict $locdict
fi

echo "--- Searching for OOV words ..."
awk 'NR==FNR{words[$1]; next;} !($1 in words)' \
  $locdict/$dict $locdata/vocab-full.txt |\
  egrep -v '<.?s>' > $locdict/vocab-oov.txt

awk 'NR==FNR{words[$1]; next;} ($1 in words)' \
  $locdata/vocab-full.txt $locdict/$dict |\
