...
#my @spkr_dirs = grep {-d "$data_base/$dataset/aac/$_" } readdir($dh);
closedir $dh;
...
foreach (@spkr_dirs) {
  ...
  #my @rec_dirs = grep {-d "$data_base/$dataset/aac/$spkr_id/$_" } readdir($dh);
  closedir $dh;
  ...
  foreach (@rec_dirs) {
    ...
    foreach (@files) {
      my $name = $_;
      my $wav = "ffmpeg -v 8 -i $data_base/$dataset/aac/$spkr_id/$rec_id/$name.m4a -f wav -ar 16000 -acodec pcm_s16le -|";
