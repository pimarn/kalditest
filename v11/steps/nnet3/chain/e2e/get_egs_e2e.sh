...
echo "2nd phase"
echo "file= $dir/valid_uttlist"
echo "frame = $frames_per_eg"
echo "len= $len_uttlist"
echo "subset= $num_utts_subset"


if [ $len_uttlist -lt $num_utts_subset ]; then
  #echo "Number of utterances which have length at least $frames_per_eg is really low. Please check your data." && exit 1;
  echo "Number of utterances which have length at least $frames_per_eg is really low. Please check your data." 
fi
...
echo "frame_per_iter= $frames_per_iter"

num_archives=$[$num_frames/$frames_per_iter+1]
echo "num_achive= $num_archives"
